...
export default function TeamTabContent({ teamId, categories, items, projectId }: TeamTabContentProps) {
    ...
    // Estado local para Items (Optimistic UI)
    const [localItems, setLocalItems] = useState<QAItem[]>(items)

    // Sincronizar localItems quando a prop items mudar (ex: refresh do servidor)
    useEffect(() => {
        setLocalItems(items)
    }, [items])
    ...
    const handleStatusChange = async (itemId: string, newStatus: string) => {
        ...
        const typedNewStatus = newStatus as QAItemStatus

        // 1. Optimistic Update
        setLocalItems(prev => prev.map(i => i.id === itemId ? { ...i, status: typedNewStatus } : i))

        try {
            // 2. Database Update
            const { error } = await supabase
                .from('qa_items')
                .update({ status: typedNewStatus })
                .eq('id', itemId)
            ...
            setLocalItems(prev => prev.map(i => i.id === itemId ? { ...i, status: oldStatus } : i))
            ...
            <div className="mb-6 flex flex-col items-start justify-between gap-4 xl:flex-row xl:items-center">
                ...
                <div className="scrollbar-hide flex max-w-full flex-1 items-center gap-2 overflow-x-auto pb-2 xl:pb-0">
                    <button
                        ...
                        className={`whitespace-nowrap rounded-full border px-4 py-1.5 text-xs font-semibold transition-colors ${selectedCategory === 'all'
                ...
                <div className="flex w-full items-center gap-2 xl:w-auto">
                    ...
                    <div className="ml-auto flex items-center rounded-lg border bg-card p-1 xl:ml-0">
                        <Button
                            ...
                            className={`h-7 px-2 text-xs ${viewMode === 'grid'
                        ...
                        <Button
                            ...
                            className={`h-7 px-2 text-xs ${viewMode === 'list'
                                ? 'bg-[#7900E5] text-white shadow-sm hover:bg-[#7900E5]/90'
                                : 'text-muted-foreground hover:text-foreground'
                                }`}
                        >
                        ...
                        <Button
                            ...
                            className={`h-7 px-2 text-xs ${viewMode === 'kanban'
                                ? 'bg-[#7900E5] text-white shadow-sm hover:bg-[#7900E5]/90'
                                : 'text-muted-foreground hover:text-foreground'
                                }`}
                        >
